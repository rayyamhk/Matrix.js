...
describe('should get correct difference of two matrices', () => {
  const tests = [
    ...
    },
    {
      ...
      expected: [
        [-10, -7, -33, 9, 5 - 10.2, 6 - 8.124, 3, 10, 63.1, 2],
      ],
    },
    {
      matrix1: [
      ...
      expected: [
        [1], [12], [-92.123], [4 - 1 / 3], [5 - Math.PI], [1], [6],
      ],
    },
    {
      matrix1: [],
  ...
  tests.forEach((test, idx) => {
    if (test.hasError) {
      ...
      it(`TEST: ${idx + 1}: should get correct difference of two matrices`, () => {
        expect.hasAssertions();
        const { matrix1, matrix2, expected } = test;
        const A = new Matrix(matrix1);
        const B = new Matrix(matrix2);
        expect(Matrix.subtract(A, B)._matrix).toStrictEqual(expected);
      });
    }
  });
});
